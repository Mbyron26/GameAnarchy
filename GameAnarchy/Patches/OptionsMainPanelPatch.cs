...
public static class OptionsMainPanelPatch {
    ...
    public static void OnVisibilityChangedPostfix(UIComponent comp, bool visible) {
        if (visible) {
            try {
                ManagerPool.GetOrCreateManager<Manager>().SetCategoriesOffset(comp);
            }
            catch (Exception e) {
    ...
    public static IEnumerable<CodeInstruction> AddUserModsTranspiler(IEnumerable<CodeInstruction> instructions) {
        ...
        MethodInfo getPluginsInfoInOrder = AccessTools.Method(typeof(PluginTools), nameof(PluginTools.GetPluginsInfoSortByName));
    ...
    private static string GetModUpdatedDate(PluginManager.PluginInfo pluginInfo) {
        ...
        if (DateTime.Equals(updatedTime, DateTime.MinValue)) {
            ...
            if (pluginInfo.publishedFileID.Equals(new ColossalFramework.PlatformServices.PublishedFileId(ulong.MaxValue))) {
                LogManager.GetLogger().Info($"Plugin [{pluginInfo.name}] is a local mod, get last write time date: {updatedTime}");
            }
            else {
                LogManager.GetLogger().Info($"Plugin [{pluginInfo.name}] updated time is not initialized yet, get last write time date: {updatedTime}");
            }
    ...
    private static string Localize(string key) => ModLocalizationManager.Localize(key);
