...
public partial class Manager {
    ...
    public void InitAchievements(LoadMode loadMode) {
        ...
        if (loadMode == LoadMode.NewGame || loadMode == LoadMode.LoadGame || loadMode == LoadMode.NewGameFromScenario || loadMode == LoadMode.LoadScenario) {
            ...
            if (unlockingPanel is null) {
                InternalLogger.Log("Init achievements failed, couldn't find UnlockingPanel");
            } else {
                InternalLogger.Log($"{loadMode} mode, init UnlockingPanel");
                var tabstrip = unlockingPanel.Find<UITabstrip>("Tabstrip");
                button = tabstrip.Find<UIButton>("Achievements");
            }
            UpdateAchievements();
        } else {
            InternalLogger.Log("Not Game mode, do not init UnlockingPanel");
            UpdateAchievements();
        }
    }
    ...
    public void UpdateAchievements() {
        ...
        try {
            ...
            InternalLogger.Log($"Update achievements, status: {isEnabled}");
        } catch (Exception e) {
            InternalLogger.Exception($"Update achievements status failed", e);
        }
    }
