    ...
    public class CompatibilityCheck {
        ...
        private static List<string> GetIncompatibleModsInfo() {
            ...
            if (DetectedIncompatibleMods.Count == 0) {
                ...
                List<string> result = new();
                foreach (var item in DetectedIncompatibleMods) {
                    result.Add($"[{item.name}] - " + (item.inclusive ? (ModName + " " + CommonLocalize.CompatibilityCheck_SameFunctionality) : CommonLocalize.CompatibilityCheck_Incompatible) + (item.useInstead is null ? "" : string.Format(CommonLocalize.CompatibilityCheck_UseInstead, item.useInstead)));
                }
                return result;
            }
        }
