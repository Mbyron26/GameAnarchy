    ...
    public class CompatibilityMessageBox : MessageBoxBase {
        public CompatibilityMessageBox() {
            if (CompatibilityCheck.IncompatibleModsInfo.Count > 0) {
                AddButtons(1, 2, CommonLocale.MessageBox_OK, Close);
                AddButtons(2, 2, CommonLocale.CompatibilityMessageBox_Unsubscribe, UnsubscribeMod);
            } else {
        ...
        public void Initialize(string modName) {
            ...
            label.textAlignment = UIHorizontalAlignment.Center;
            label.text = $"{modName} {CommonLocale.OptionPanel_CompatibilityCheck}";
